...
One file per SAR scene. Filename format changed between 2023 and 2024:
  ...
  - After: SAR-Wind-{POL}-{lat}{N|S}-{lon}{E|W}_v{maj}r{min}_rsat2_
    s{start}_e{end}_c{created}.nc
    e.g. SAR-Wind-HH-64N-174E_v3r0_rsat2_s202606040552510_e202606040554070_c202606041745293.nc

Both contain a scene-center lon/lat in the filename, used as a coarse
...
# ._passes_ncml_check). Therefore, using a generous bbox.
_BBOX_PAD_DEG = 5.0
...
def _signed(value: float, hem: str, positive: str) -> float:
    """
    Convert an unsigned coordinate *value* to its signed form.

    *hem* is the hemisphere letter parsed from the filename (e.g.
    "N"/"S" for latitude, "E"/"W" for longitude); *positive* is the
    letter denoting that axis's positive hemisphere ("N" or "E").
    Returns *value* unchanged if *hem* equals *positive*, otherwise
    its negation.
    """
...
def _lon_within_padded_bbox(lon: float, min_lon: float, max_lon: float) -> bool:
    """
    True if *lon* (normalized to [-180, 180]) falls within
    [min_lon, max_lon] padded by _BBOX_PAD_DEG.

    A naive `min_lon - PAD <= lon <= max_lon + PAD` check fails near the
    antimeridian: padding max_lon=180 by 5 degrees gives an unreachable
    upper bound of 185, even though a candidate at -178 is really just 2
    degrees past the edge. When the padded window exceeds +-180, this
    also accepts the wrapped-around slice on the other side (e.g.
    lon <= -175).

    This only concerns the pad's own overflow -- callers (see
    RADARSAT2WindDownloader.download) pre-split antimeridian-crossing
    requests via split_antimeridian_bbox, so min_lon <= max_lon always
    holds here.
    """
...
def _parse_ncml_bbox(ncml_xml_text: str) -> Optional[Tuple[float, float, float, float]]:
    """
    Parse a THREDDS NCML metadata document (the `/ncml/{urlPath}` service) 
    and return (lon_min, lon_max, lat_min, lat_max), or None if the four 
    geospatial_*_min/max attributes are not found anywhere in it.

    New-era granules carry these as root-level global attributes. Old-era
    granules' raw NetCDF files carry no such attributes, but THREDDS' NCML 
    service still reports them -- auto-computed server-side from the 
    coordinate data -- nested inside a <group name="CFMetadata"> element 
    instead. root.iter() finds either location regardless of nesting; 
    taking the *first* match of each attribute name in document order 
    picks up the root-level value when present (it always precedes any 
    nested group) and falls back to the CFMetadata-group value otherwise,
    with no era-conditional branching needed.
    """
...
class RADARSAT2WindDownloader:
    ...
    def _download_window(self, min_lon, max_lon, min_lat, max_lat, start: str, end: str) -> list[Path]:
        ...
        # midnight, matching normalize_datetime and every other exact-instant
        # comparison in this pipeline (e.g. the in-situ download window). A
        # granule timestamped later the same day therefore falls genuinely
        # outside the requested window and must not match; end_dt is used as
        # given, with no "widen to the whole day" special-casing.
        ...
        if self.dry_run:
            # The dry-run still performs each touched month's (lightweight)
            # catalog.xml query above, so it reports real candidate availability
            # but never runs the per-candidate NCML check or downloads a full
            # ~38MB fileServer file.
            print(
    ...
    def _passes_ncml_check(url_path: str, min_lon, max_lon, min_lat, max_lat) -> bool:
        ...
        metadata is unavailable/unparseable. Failing open in the latter 
        case avoids silently dropping a real granule due to a transient 
        metadata-service issue. The cost of a false positive is one
        unnecessary ~38MB download, not a science error, since nothing
        downstream re-checks the footprint after download.
        """
        try:
            with prefer_ipv4_dns(), urllib.request.urlopen(
