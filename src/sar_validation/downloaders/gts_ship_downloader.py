"""
Download ship synoptic wind observations from the WMO Global
...
class ShipDownloader:
    """
    Download ship synoptic wind observations via MARS, one BUFR file per
    ...
    def download(
        ...
        log output, but MARS's ship obstype has no bbox request facet --
        the whole day's global ship reports are fetched and cropped to the
        ...
        while day <= window_end.date():
            ...
            if self.dry_run:
                logger.info(
                    "  [dry-run] would download GTS ship obs for %s "
    ...
    def _bufr_path_for_day(self, day: date) -> Path:
        return self.output_dir / f"gts_ship_{day.strftime('%Y%m%d')}.bufr"
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Download GTS ship synoptic wind observations via MARS."
    )
    ...
    p.add_argument("--output-dir", type=Path, default=None,
                   help="Output directory (default: data/<timerange>_<bounds>/gts_ship).")
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    out_dir = Path(args.output_dir) if args.output_dir else (
        build_output_dir(args.start, args.end, args.min_lon, args.max_lon,
                         args.min_lat, args.max_lat) / "gts_ship"
    )
    out_dir.mkdir(parents=True, exist_ok=True)
    dl = ShipDownloader(output_dir=out_dir, dry_run=args.dry_run)
    dl.download(
