...
-----
  ...
  # Store credentials in the OS keyring (eumdac | osi_saf | gportal | smos | hsaf)
...
Examples:
  ...
  sar-validate --recipe recipes/wind_validation.yaml --plot
    ...
    parser.add_argument(
        "--set-credential",
        metavar="SERVICE",
        choices=["eumdac", "osi_saf", "gportal", "smos", "earthdata", "hsaf"],
        help="Prompt for a username/password and store them in the OS keyring "
             "for SERVICE (eumdac | osi_saf | gportal | smos | earthdata | hsaf)",
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def _set_credential(name: str) -> None:
    ...
    Backs ``sar-validate --set-credential {eumdac,osi_saf,gportal,smos,earthdata,hsaf}``.
    """
    import getpass
