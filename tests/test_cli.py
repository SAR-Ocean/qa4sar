...
class TestStatsAlreadyComputed:
    def _write_collocation_and_stats(self, tmp_path, coll_mtime, stats_mtime):
        collocation_ds = xr.Dataset({
            "sar_oswTotalHs": ("collocation", [1.4, 1.5]),
            "val_VAVH":       ("collocation", [1.42, 1.48]),
            "val_source":     ("collocation", ["altimeter", "altimeter"]),
        })
        coll_path = tmp_path / "collocation_results.nc"
        collocation_ds.to_netcdf(coll_path)
        os.utime(coll_path, (coll_mtime, coll_mtime))

        stats_ds = xr.Dataset({"bias": ("source", [0.02])}, coords={"source": ["altimeter"]})
        stats_path = tmp_path / "validation_statistics_oswTotalHs_vs_SWH.nc"
        stats_ds.to_netcdf(stats_path)
        os.utime(stats_path, (stats_mtime, stats_mtime))

    def test_returns_false_when_collocation_results_is_newer_than_stats(self, tmp_path):
        """A validation_statistics file left over from a previous
        collocation run -- e.g. collocation_results.nc got regenerated by a
        rerun, a bug fix, or a manual repair without Step 4 also rerunning
        -- must trigger a recompute rather than silently serving numbers
        that no longer match the current collocation results."""
        recipe = _waves_recipe(["WV", "SM"])
        now = time.time()
        self._write_collocation_and_stats(tmp_path, coll_mtime=now, stats_mtime=now - 3600)

        assert cli._stats_already_computed(recipe, tmp_path) is False

    def test_returns_true_when_stats_are_newer_than_collocation_results(self, tmp_path):
        recipe = _waves_recipe(["WV", "SM"])
        now = time.time()
        self._write_collocation_and_stats(tmp_path, coll_mtime=now - 3600, stats_mtime=now)

        assert cli._stats_already_computed(recipe, tmp_path) is True
