...
**`owiWindQuality`/`owiInversionQuality` gate OWI wind retrievals, independently of `owiMask`.** `_extract_owi_grid_data` NaNs out `owiWindSpeed`/`owiWindDirection` wherever either flag reports low/poor quality (`owiWindQuality` >= 2, `owiInversionQuality` >= 2), on top of the land masking above. A flag that is simply missing for one cell (present in the product but NaN there) does not reject that cell by itself — the other flag governs, because a cell can be valid and non-land while one flag's metric was simply never computed for it. A cell is only rejected on NaN grounds when both flags are present in the product and both are NaN for that cell. A flag that is entirely absent from the product (not the "some cells are NaN" case, but the variable does not exist at all) never contributes to rejection, so a product carrying only one flag, or neither, is masked using only the flags actually present. Counts and fractions are tracked separately from land masking, as `owi_quality_masked_pixel_count`/`owi_quality_masked_pixel_fraction`.
